...
public class ThirstBarRenderer {
   ...
   public static IGuiOverlay THIRST_OVERLAY = (gui, poseStack, partialTicks, screenWidth, screenHeight) -> {
      ...
      if (!isMounted && !minecraft.f_91066_.f_92062_ && gui.shouldDrawSurvivalElements()) {
         if (checkIfPlayerIsVampire && Helper.isVampire(gui.getMinecraft().f_91074_)) {
            cancelRender = true;
            return;
         }

         if (minecraft.f_91074_.m_6084_() && !((IThirst)minecraft.f_91074_.getCapability(ModCapabilities.PLAYER_THIRST).orElse((Object)null)).getShouldTickThirst()) {
            cancelRender = true;
            return;
         }
   ...
   public static void render(ForgeGui gui, int width, int height, GuiGraphics guiGraphics) {
      minecraft.m_91307_().m_6180_("thirst");
      ...
      int top = height - gui.rightHeight + (Integer)ClientConfig.THIRST_BAR_Y_OFFSET.get();
      gui.rightHeight += 10;
      ...
      for(int i = 0; i < 10; ++i) {
         ...
         int x = left - i * 8 - 9;
         int y = top;
         if ((float)PLAYER_THIRST.getQuenched() <= 0.0F && gui.m_93079_() % (level * 3 + 1) == 0) {
