...
@Mixin(
   value = {IHaveGoggleInformation.class},
   remap = false
)
public interface MixinIHaveGoggleInformation {
         ...
         if (tank.getTanks() == 0) {
            ...
            for(int i = 0; i < tank.getTanks(); ++i) {
               ...
               if (!fluidStack.isEmpty()) {
                  ...
                  CreateLang.builder().add(CreateLang.number((double)fluidStack.getAmount()).add(mb).style(ChatFormatting.GOLD)).text(ChatFormatting.GRAY, " / ").add(CreateLang.number((double)tank.getTankCapacity(i)).add(mb).style(ChatFormatting.DARK_GRAY)).forGoggles(tooltip, 1);
                  isEmpty = false;
               }
            }
            ...
            if (tank.getTanks() > 1) {
               ...
               CreateLang.translate("gui.goggles.fluid_container.capacity", new Object[0]).add(CreateLang.number((double)tank.getTankCapacity(0)).add(mb).style(ChatFormatting.GOLD)).style(ChatFormatting.GRAY).forGoggles(tooltip, 1);
               return true;
            }
         }
      }
   ...
   default ChatFormatting getPurityColor(int purity) {
      if (purity == 3) {
         ...
         return purity == 0 ? ChatFormatting.DARK_GRAY : ChatFormatting.GRAY;
      }
   }
}
