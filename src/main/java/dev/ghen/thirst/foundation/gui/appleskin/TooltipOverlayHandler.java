...
public class TooltipOverlayHandler {
   ...
   private static boolean shouldShowTooltip(ItemStack hoveredStack) {
      if (hoveredStack.m_41619_()) {
         ...
         return !shouldShowTooltip ? false : ThirstHelper.itemRestoresThirst(hoveredStack);
      }
   }
   ...
   static class FoodTooltip implements TooltipComponent {
      ...
      FoodTooltip(ItemStack itemStack) {
         ...
         this.biggestSaturationIncrement = (float)ThirstHelper.getQuenched(itemStack);
         ...
         if (TooltipOverlayHandler.shouldShowTooltip(itemStack)) {
            ...
            if (gui != null) {
               ...
               if (this.foodTooltip.hungerBarsText != null) {
                  ...
                  offsetX += 18;
                  poseStack.m_85836_();
                  poseStack.m_252880_((float)offsetX, (float)offsetY, 0.0F);
               ...
               offsetY += 10;
               float modifiedSaturationIncrement = (float)ThirstHelper.getQuenched(itemStack);
               float absModifiedSaturationIncrement = Math.abs(modifiedSaturationIncrement);
               offsetX = x + (this.foodTooltip.saturationBars - 1) * 7;
               ...
               for(int i = 0; i < this.foodTooltip.saturationBars * 2; i += 2) {
                  ...
                  guiGraphics.m_280398_(TooltipOverlayHandler.modIcons, offsetX, offsetY, 0, effectiveSaturationOfBar >= 1.0F ? 21.0F : ((double)effectiveSaturationOfBar > (double)0.5F ? 14.0F : ((double)effectiveSaturationOfBar > (double)0.25F ? 7.0F : (effectiveSaturationOfBar > 0.0F ? 0.0F : 28.0F))), modifiedSaturationIncrement >= 0.0F ? 27.0F : 34.0F, 7, 7, 256, 256);
                  if (shouldBeFaded) {
                     RenderSystem.setShaderColor(1.0F, 1.0F, 1.0F, 1.0F);
                  }
